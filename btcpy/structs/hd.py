...
class ExtendedKey(HexSerializable, metaclass=ABCMeta):
    ...
    @classmethod
    def decode(cls, string, constants):

        if string[0] != constants.xkeys_prefixes:
            raise ValueError(
                'Expected key prefix {0} but found {1}'.format(
                    constants.xkeys_prefixes,
                    string[0],
                )
            )
    ...
    def encode(self, constants):
        return b58encode_check(bytes(self.serialize(constants)))

    def serialize(self, constants):
        cls = self.__class__
        result = Stream()
        result << cls.get_version(constants)
